...
def testresult(c):
	...
	if 3 in c:
		subprocess.call(["patch", "succ.c", "-i", "patch/conf3.patch"])
		f.write('3 ')
	else:
		f.write('. ')
	...
	if 6 in c:
		subprocess.call(["patch", "succ.c", "-i", "patch/conf6.patch"])
		f.write('6 ')
	else:
		f.write('. ')
...
#delta debugging
#searches two failure-inducing changes
#with inference
def dd(c, r):
	c1 = c[:len(c)//2]
	c2 = c[len(c)//2:]
	...
	elif testresult(c1Ur) == False:
		dd(c1, r)

	elif testresult(c2Ur) == False:
		dd(c2, r)
